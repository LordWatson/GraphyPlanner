...
export default function ClientInviteShow({
    token,
    client,
    email,
}: {
    ...
    return (
        <>
            <Head title={`Join the ${client.name} portal — Graphy`} />
                ...
                <main className="mx-auto flex w-full max-w-md flex-1 flex-col gap-6 px-6 pb-16">
                    <div className="flex flex-col gap-2">
                        ...
                        <h1 className="font-serif text-2xl font-bold tracking-tight sm:text-3xl">
                            <span className="text-gradient-brand">{client.name}&apos;s portal</span>
                        </h1>
                        <p className="flex items-center gap-1.5 text-sm text-muted-foreground">
                            <UserPlus className="size-4" />
                            Set a password to finish creating your account for {email}
                        </p>
                    </div>

                    <Form
                        action={`/client-invite/${token}`}
                        method="post"
